...
const zh = {
  ...
  docs_cmd_local: "离线解密从网盘下载的备份(.zip 或解压后的目录),无需登录:提示输入助记词后,为每个条目写出一个 JSON,并生成一个自包含的 index.html;全程不上传,只留在本机。",
...
const en: typeof zh = {
  ...
  docs_cmd_local: "Decrypt a backup downloaded from your netdisk (a .zip or its extracted directory) fully offline — no login. Prompts for the recovery phrase, then writes one JSON per item plus a self-contained index.html; nothing is uploaded.",
