...
- **新增**:`apps/desktop`(Tauri)
  ...
  - 端口配置:配置文件 / 设置页可改,默认 35291;读取顺序 配置 > `KEYMASK_LOCAL_PORT` > 35291;改动需重启 sidecar。
  - 启动后把 `{ port, token }` 写到 **`~/.keymask/local.json`**(loopback 鉴权 token,供 CLI 读 —— 与 003 对齐)。
- **更新**:`packages/db`
...
- [ ] `~/.keymask/local.json` 在桌面启动后存在且含 `{ port, token }`。
...
- `~/.keymask/local.json` 的路径/字段必须与 003 CLI 严格一致。
...
### 做了什么
- **可插拔 token 存储**(`packages/db`):`db.ts` 改惰性 `getDb()`(无 DATABASE_URL 不再 import 即抛);新增 `token-store.ts`(接口 + 按 `KEYMASK_TOKEN_STORE` 分派)、`token-store-postgres.ts`(原 Drizzle 逻辑,惰性连接)、`token-store-json.ts`(本地 JSON 文件,0600,`KEYMASK_TOKEN_FILE` 或 `~/.keymask/tokens.json`);`storage-accounts.ts` 改为委托,新增 `listStorageAccounts()`;`index.ts` 导出 `getDb`/`listStorageAccounts`。
- **本地接口无 cookie 鉴权**(`apps/web`):`google.ts` 抽出 `getConnectedGoogleBySub()`;`storage.ts` 抽出 `wrapGoogle()` + 新增 `getConnectedStorageByLocalAuth(request)`(校验 `x-keymask-token` → 按唯一 Google 账号解析)+ `getStorageForRequest(request)`(先 cookie 后本地 token);`/api/files` 与 `/api/files/content` 改用 `getStorageForRequest`。
- **桌面 v1 隐藏百度**:`Landing` 加 `hideBaidu`,`page.tsx` 按 `KEYMASK_DESKTOP===1` 传入;launcher 设该环境变量。
- **sidecar**:`next.config.ts` 加 `KEYMASK_STANDALONE` 触发 standalone(+ transpile `@keymask/vault`);`apps/desktop/sidecar/launch.mjs`(定端口 desktop.json>env>35291、生成 token、写 `~/.keymask/local.json`、配 `GOOGLE_REDIRECT_URI`/`HOSTNAME=127.0.0.1`、起 server.js)、`paths.mjs`、`scripts/bundle-sidecar.mjs`。
...
- [x] `~/.keymask/local.json` 由 launcher 写出含 `{port, token}`(实测:KEYMASK_LOCAL_PORT=40000 → port=40000 + 随机 token,0600)。
- [x] 不依赖 Postgres:standalone server 在 `KEYMASK_TOKEN_STORE=json` 下启动、首页 200、token 存储 JSON 往返(实测,全程无 DATABASE_URL)。
- [x] standalone 产物:`KEYMASK_STANDALONE=1 next build` 产出 `.next/standalone/apps/web/server.js`;launcher/bundle 路径对齐。
