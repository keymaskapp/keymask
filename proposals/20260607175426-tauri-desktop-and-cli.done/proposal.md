# Tauri 桌面版 + keymask CLI

> Created: 2026-06-07

## 结论

- 一句话方案:把 vault 数据层抽成 `@keymask/vault`(可插拔 transport + cache,E2E 逻辑共用),桌面端用 Tauri 内置 Node sidecar 跑现有 Next.js 于可配端口(默认 35291)、token 存储本地化为 JSON 文件;`@keymask/cli` 作为**自带 E2E 的密文中转客户端**,默认访问 localhost:35291 管理 items。**桌面 / CLI v1 仅支持 Google Drive,百度后置。**
- 完成的可观测信号:`keymask ls` 在终端列出当前保险库条目标题;`keymask new/get/rm` 与桌面 UI 互见(同一密文、解密一致);本地接口 `:35291/api/files` 只经手密文(明文/助记词/密钥从不出现在 sidecar 与 CLI↔sidecar 流量里);桌面无 Postgres、无外部 DB 依赖。
...
- `@keymask/crypto`(`packages/crypto/src/index.ts`)只用 `globalThis.crypto.subtle` + `@noble`/`@scure`,无 `node:crypto` → Node 20+ 直接可跑,CLI 端 crypto 零改动。
...
- 助记词派生的「记住本设备」存的是浏览器 IndexedDB 里 non-extractable CryptoKey(`apps/web/src/lib/key-store.ts`),CLI 读不到 → CLI 自管 `~/.keymask`。
...
- CLI 密钥自管 `~/.keymask`(选 A 不选共享 keychain)—— 职责清晰、不依赖桌面解锁态;助记词经 `KEYMASK_MNEMONIC`/交互输入,`keymask login` 后落本机加密态。
- 本地接口鉴权约定文件 = **`~/.keymask/local.json`**(桌面写 `{ port, token }`,CLI 读)—— 单一周知路径,两端对齐;CLI 默认端口与 token 都从此文件取,缺失时回退 35291 并报"桌面未运行"。

## Plans 拆分

| 编号 | 标题 | 路径 | 依赖 | 状态 |
|---|---|---|---|---|
| 001 | 抽取共享 vault 数据层 `@keymask/vault` | `plans/001-shared-vault-package.done.md` | - | 已完成 |
| 002 | Tauri 桌面版 + Node sidecar + 本地接口 | `plans/002-tauri-desktop-sidecar.done.md` | 001 | 已完成 |
| 003 | `@keymask/cli` 命令行 | `plans/003-keymask-cli.done.md` | 001, 002 | 已完成 |
