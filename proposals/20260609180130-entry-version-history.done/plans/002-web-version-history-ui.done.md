...
- **更新**:
  ...
  - `apps/web/src/lib/vault.ts`:re-export `listVersions`/`openVersion`/`openFileVersion`/`restoreVersion` 与 `VersionMeta` 类型(透传 `@keysark/vault`)。
...
- 所有新布局容器经 `@/lib/test-id` 的 `testId()`,kebab-case 稳定命名(规则 4);shadcn 组件只从 `@keysark/ui` 取(规则 1)。
...
- [x] `pnpm --filter @keysark/web typecheck` + `build` 通过。
